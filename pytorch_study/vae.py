...
def loss_function(recon_x, x, mu, logvar):
    ...
    # reconstrunction loss 入力画像をどのくらい正確に復元できたか.
    # 数式では対数尤度の最大化だが交差エントロピーlossの最小化と等価
    recon = F.binary_cross_entropy(recon_x, x.view(-1, 784), size_average=False)

    # 潜在空間zに対する正則化項
    # p(z|x) がN(0, I)に近くなる(KL-Divergenceが小さくなる)ようにする
    kid = - 0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
...
def train(epoch):
    ...
    # loss_function()は平均ではなく全サンプルの合計lossを返すので
    # サンプル数で割る
        ...
        if epoch % 10 == 0:
            if batch_idx == 0:
                ...
                save_image(comparison.data.cpu(),
                           '{}/reconstrunction_{}.png'.format(out_dir, epoch), nrow=n)
